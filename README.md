...
#define ROGU_SOURCE_LOCATION     // Enable source location in log output
#define ROGU_TIMESTAMP           // Enable UTC timestamps in log output
#include "rogu.hpp"
```
...
| `rogu::msg_only`   | Suppress all fields except the message body; useful for continuation lines after `no_break` |
| `rogu::no_time`    | Suppress the timestamp for this call regardless of formatter settings. Requires `ROGU_TIMESTAMP`. |
| `rogu::force_time` | Force the timestamp for this call, overriding `no_time`. Requires `ROGU_TIMESTAMP`. |
...
```
Multiple streams are written simultaneously. Each stream has its own format string and its own per-stream level settings. A typical setup logs human-readable output to `std::cout` and a persistent record to a file.
...
The `{msg}` token marks the position where `<<` stream continuations are inserted. Content written via `<<` appears at that position, with any text following `{msg}` in the format string appended afterwards by the stream wrapper's destructor. If `{msg}` is absent from the format string, the message body is not written.
...
Per-stream control requires `ROGU_LOGLEVEL_PER_STREAM`. A level must first be delegated before per-stream settings take effect:
```cpp
